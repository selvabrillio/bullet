...
Cube::Cube()
{
	SetTransform( ref new Transform(0,0,0,1,0.f, 0.f, 0.f ));
}
...
void Cube::Create(ComPtr<ID3D11DeviceContext1> &d3dDeviceContext)
{
	this->cube = GeometricPrimitive::CreateCube(d3dDeviceContext.Get(),1,true);
}

void Cube::Draw(CXMMATRIX world, CXMMATRIX view, CXMMATRIX projection, FXMVECTOR color, ID3D11ShaderResourceView* texture)
{
	this->cube->Draw(world * this->modelMatrix, view, projection, color, texture);
}
